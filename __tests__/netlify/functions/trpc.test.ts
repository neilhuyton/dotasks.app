...
describe('Netlify Function: trpc', () => {
  ...
  const TRPC_ENDPOINT = 'http://localhost:8888/.netlify/functions/trpc';

  const createTrpcBody = (path: string, input: Record<string, unknown>): TrpcRequestBody => ({
    json: { path, input },
  });
  ...
  it('successfully registers a new user', async () => {
    ...
    const response = await fetch(TRPC_ENDPOINT, {
      ...
      body: JSON.stringify(createTrpcBody('register', {
        email: 'newuser@example.com',
