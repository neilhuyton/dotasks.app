// __tests__/hooks/useVerifyEmailPage.test.tsx
import {
  describe,
  it,
  expect,
...
describe("useVerifyEmailPage Hook", () => {
  const setup = async (
  ...
  it("verifies email successfully with valid token", async () => {
    vi.mocked(useSearch).mockReturnValue({
      token: TEST_VERIFICATION_TOKENS.DELAYED_SUCCESS,
    });
  ...
  it("handles invalid token with error message", async () => {
    vi.mocked(useSearch).mockReturnValue({
      token: TEST_VERIFICATION_TOKENS.INVALID,
    });
